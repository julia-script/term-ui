...
const Type = enum {
    ...
    fn get(cp: u21) Type {
        ...
        switch (wb) {
            .ALetter => ty = .aletter,
            .CR => ty = .cr,
            .Double_Quote => ty = .dquote,
            .Extend => ty = .extend,
            .ExtendNumLet => ty = .extendnumlet,
            .Format => ty = .format,
            .Hebrew_Letter => ty = .hletter,
            .Katakana => ty = .katakana,
            .LF => ty = .lf,
            .MidLetter => ty = .midletter,
            .MidNum => ty = .midnum,
            .MidNumLet => ty = .midnumlet,
            .Newline => ty = .newline,
            .Numeric => ty = .numeric,
            .Regional_Indicator => ty = .regional,
            .Single_Quote => ty = .squote,
            .WSegSpace => ty = .wsegspace,
            .ZWJ => ty = .zwj,
            .Other => ty = .any,
        }
...
pub const WordIterator = struct {
    ...
    pub fn init(str: []const u8) !Self {
        ...
        return Self{ .cp_iter = unicode.Utf8Iterator{ .bytes = str, .i = 0 } };
    }
    ...
    fn emit(self: Self, start_token: Token, end_token: Token) Word {
        const start = start_token.offset;
        const end = end_token.offset + end_token.len;
