...
/// In nowrap mode, each forced break creates a new line regardless of available width
/// In wrap mode, use normal line breaking algorithm
pub fn buildLinesWithWrapMode(self: *Self, wrap_mode: TextWrapMode) !void {
    if (wrap_mode == .nowrap) {
        try self.buildLinesNowrap();
    } else {
        try self.buildLinesWrap();
    }
}
...
pub fn isForcedLineBreak(codepoint: u21) bool {
    return switch (codepoint) {
...
pub fn preserveForcedLineBreaks(self: *Self, text: []const u8) !void {
    if (text.len == 0) return;
