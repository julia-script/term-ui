...
pub fn collectFlexLines(
    ...
    if (!constants.is_wrap) {
        var lines: std.ArrayList(types.FlexLine) = .empty;
        try lines.ensureTotalCapacity(allocator, 1);
    ...
    switch (main_available_space) {
        ...
        .min_content => {
            var lines: std.ArrayList(types.FlexLine) = .empty;
            try lines.ensureTotalCapacity(allocator, flex_items.items.len);
        ...
        .definite => |definite| {
            ...
            const main_axis_gap = dir.getMain(constants.gap);
            var is_new_row = true;
            var index: usize = 0;
            ...
            while (index < flex_items.items.len) {
                ...
                const item_main_size = dir.getMain(current_item.hypothetical_outer_size);
                line_length += item_main_size + gap_contribution;

                if (line_length > definite and !is_new_row) {
                    try lines.append(allocator, .{ .items = flex_items.items.ptr[start_range_index..index], .cross_size = 0.0, .offset_cross = 0.0 });
            ...
            if (start_range_index < flex_items.items.len) {
                try lines.append(allocator, .{ .items = flex_items.items.ptr[start_range_index..flex_items.items.len], .cross_size = 0.0, .offset_cross = 0.0 });
            }
