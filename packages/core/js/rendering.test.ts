...
describe("Rendering System Tests", () => {
  ...
  it("should render a simple button to the screen", () => {
    ...
    // Test getElementById
    const foundButton = mod.Tree_getElementById(
      tree,
      "my-button",
    );
    ...
    const hitResult = mod.Tree_hitTest(
      tree,
      50,
      15,
    ); // Center of button
    ...
    const caretPos =
      mod.Tree_caretPositionFromPoint(
        tree,
        45,
        15,
      );
  ...
  it("should render a complex layout with multiple elements", () => {
    ...
    const contentText = mod.Tree_createTextNode(
      tree,
      "This is some content text that should wrap nicely within the container.",
    );
    mod.Tree_appendChild(
      tree,
      content,
    ...
    expect(
      mod.Tree_getElementById(tree, "container"),
    ).toBe(container);
    expect(
      mod.Tree_getElementById(tree, "header"),
    ).toBe(header);
    expect(
      mod.Tree_getElementById(tree, "content"),
    ).toBe(content);
    expect(
      mod.Tree_getElementById(tree, "submit-btn"),
    ).toBe(button);

    // Test hit testing on different elements
    const hitHeader = mod.Tree_hitTest(
      tree,
      150,
      40,
    );
    const hitContent = mod.Tree_hitTest(
      tree,
      150,
      100,
    );
    const hitButton = mod.Tree_hitTest(
      tree,
      60,
      180,
    );
    ...
    console.log(
      `Hit results - Header: ${hitHeader}, Content: ${hitContent}, Button: ${hitButton}`,
    );

    // Test that we can get node at position with renderer
    const nodeAtButton = mod.Renderer_getNodeAt(
      renderer,
      tree,
      60,
      180,
  ...
  it("should handle style property updates with proper invalidation", () => {
    ...
    const element = mod.Tree_createNode(
      tree,
      "display: block; width: 100px; height: 50px;",
    );
    const text = mod.Tree_createTextNode(
      tree,
      "Test Element",
    );
    ...
    console.log(
      `Invalidation status after style changes: ${status}`,
    );
  ...
  it("should test caret positioning and text interaction", () => {
    ...
    const textContainer = mod.Tree_createNode(
      tree,
      "display: block; width: 200px; padding: 10px; background-color: lightyellow;",
    );

    const paragraph1 = mod.Tree_createTextNode(
      tree,
      "First paragraph of text. ",
    );
    const paragraph2 = mod.Tree_createTextNode(
      tree,
    ...
    console.log(
      `Start: node=${caretStart?.node}, offset=${caretStart?.offset}`,
    );
    console.log(
      `Middle: node=${caretMiddle?.node}, offset=${caretMiddle?.offset}`,
    );
    console.log(
      `End: node=${caretEnd?.node}, offset=${caretEnd?.offset}`,
    );
